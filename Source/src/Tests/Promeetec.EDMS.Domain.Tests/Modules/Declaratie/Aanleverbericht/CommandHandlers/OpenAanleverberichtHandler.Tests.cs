...
[TestFixture]
public class OpenAanleverberichtHandlerTests : TestFixtureBase
{
    ...
    [Test]
    public async Task Should_open_aanleveringbericht()
    {
        ...
        var command = Fixture.Build<OpenAanleverbericht>()
        ...
        var sut = new OpenAanleverberichtHandler(_repository);
        await sut.Handle(command);

        var dbEntity = await _context.Aanleverberichten.FirstOrDefaultAsync(x => x.Id == command.Id);

        Assert.NotNull(dbEntity);
        Assert.AreEqual(AanleverberichtStatus.Open, dbEntity?.AanleverberichtStatus);
    }
}