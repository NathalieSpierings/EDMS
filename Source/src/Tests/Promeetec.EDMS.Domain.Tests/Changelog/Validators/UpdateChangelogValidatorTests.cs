...
{
    private UpdateChangelogPostValidator _validator;

    [SetUp]
    public void Setup()
    {
        _validator = new UpdateChangelogPostValidator();
    }

    [Test]
    public void Should_have_validation_error_when_title_is_empty()
    {
        var command = Fixture.Build<UpdateChangelogPost>().With(x => x.Title, string.Empty).Create();
    ...
    {
        var command = Fixture.Build<UpdateChangelogPost>().With(x => x.Title, new string('*', 222)).Create();
        var result = _validator.TestValidate(command);
        result.ShouldHaveValidationErrorFor(x => x.Title);
    }
    ...
    {
        var command = Fixture.Build<UpdateChangelogPost>().With(x => x.Description, string.Empty).Create();
        var result = _validator.TestValidate(command);
    ...
    {
        var command = Fixture.Build<UpdateChangelogPost>().With(x => x.Description, new string('*', 460)).Create();
        var result = _validator.TestValidate(command);
        result.ShouldHaveValidationErrorFor(x => x.Description);
    }
    ...
    {
        var command = Fixture.Build<UpdateChangelogPost>().With(x => x.Content, string.Empty).Create();
        var result = _validator.TestValidate(command);
