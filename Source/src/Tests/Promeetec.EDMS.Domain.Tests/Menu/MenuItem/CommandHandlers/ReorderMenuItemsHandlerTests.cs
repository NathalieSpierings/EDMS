...
[TestFixture]
public class ReorderMenuItemsHandlerTests : TestFixtureBase
{
    ...
    [Test]
    public async Task Should_reinstate_menu_and_add_event()
    {
        var cmd = Fixture.Build<ReorderMenuItems>()
        ...
        var command = Fixture.Build<ReorderMenuItems>()
            .With(x => x.Id, Menu.Id)
            .With(x => x.OrganisatieId, PromeetecId)
        ...
        var sut = new ReorderMenuItemsHandler(_repository, _eventRepository);
