...
[TestFixture]
public class ReinstateMenuHandlerTests : TestFixtureBase
{
    ...
    public async Task Should_reinstate_menu_and_add_event()
    {
        ...
        var command = Fixture.Build<ReinstateMenu>()
            .With(x => x.Id, Menu.Id)
            .With(x => x.OrganisatieId, PromeetecId)
        ...
        var sut = new ReinstateMenuHandler(_repository, _eventRepository);
