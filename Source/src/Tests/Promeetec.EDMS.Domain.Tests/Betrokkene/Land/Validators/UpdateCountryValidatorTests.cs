...
[TestFixture]
public class UpdateCountryValidatorTests : TestFixtureBase
{
    private UpdateCountryValidator _validator;

    [SetUp]
    public void Setup()
    {
        _validator = new UpdateCountryValidator();
    }
    ...
    {
        var command = Fixture.Build<UpdateCountry>().With(x => x.CultureCode, string.Empty).Create();
    ...
    {
        var command = Fixture.Build<UpdateCountry>().With(x => x.CultureCode, new string('*', 51)).Create();
        var result = _validator.TestValidate(command);
        result.ShouldHaveValidationErrorFor(x => x.CultureCode);
    }
    ...
    {
        var command = Fixture.Build<UpdateCountry>().With(x => x.NativeName, string.Empty).Create();
        var result = _validator.TestValidate(command);
    ...
    {
        var command = Fixture.Build<UpdateCountry>().With(x => x.NativeName, new string('*', 129)).Create();
        var result = _validator.TestValidate(command);
        result.ShouldHaveValidationErrorFor(x => x.NativeName);
    }
}