...
[TestFixture]
public class CreateCountryValidatorTests : TestFixtureBase
{
    private CreateCountryValidator _validator;
    ...
    {
        var command = Fixture.Build<CreateCountry>().With(x => x.CultureCode, string.Empty).Create();
    ...
    {
        var command = Fixture.Build<CreateCountry>().With(x => x.CultureCode, new string('*', 51)).Create();
        var result = _validator.TestValidate(command);
        result.ShouldHaveValidationErrorFor(x => x.CultureCode);
    }
    ...
    {
        var command = Fixture.Build<CreateCountry>().With(x => x.NativeName, string.Empty).Create();
        var result = _validator.TestValidate(command);
    ...
    {
        var command = Fixture.Build<CreateCountry>().With(x => x.NativeName, new string('*', 129)).Create();
        var result = _validator.TestValidate(command);
        result.ShouldHaveValidationErrorFor(x => x.NativeName);
    }
}