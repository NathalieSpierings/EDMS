...
[TestFixture]
public class CreateMedewerkerValidatorTests : TestFixtureBase
{
    private CreateMedewerkerValidator _validator;
    ...
    {
        var command = Fixture.Build<CreateMedewerker>().Without(x => x.Adres).With(x => x.Email, new string('*', 460)).Create();
        var result = _validator.TestValidate(command);
    ...
    {
        var command = Fixture.Build<CreateMedewerker>().Without(x => x.Adres).With(x => x.Email, "email").Create();
        var result = _validator.TestValidate(command);
        result.ShouldHaveValidationErrorFor(x => x.Email);
    }
    ...
    {
        var command = Fixture.Build<CreateMedewerker>().Without(x => x.Adres).With(x => x.Functie, new string('*', 201)).Create();
        var result = _validator.TestValidate(command);
