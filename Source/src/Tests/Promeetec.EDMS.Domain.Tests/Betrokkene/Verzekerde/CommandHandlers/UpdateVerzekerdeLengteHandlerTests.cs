...
[TestFixture]
public class UpdateVerzekerdeLengteHandlerTests : TestFixtureBase
{
    ...
    [Test]
    public async Task Should_update_verzekerde_length()
    {
        ...
        var command = Fixture.Build<UpdateVerzekerdeLength>()
        ...
        var sut = new UpdateVerzekerdeLengteHandler(_repository);
        await sut.Handle(command);

        var dbEntity = await _context.Verzekerden.FirstOrDefaultAsync(x => x.Id == verzekerde.Id);

        Assert.NotNull(dbEntity);
        Assert.AreEqual(command.Lengte, dbEntity?.Lengte);
    }
}