...
[TestFixture]
public class UpdateUserProfileHandlerTests : TestFixtureBase
{
    ...
    [Test]
    public async Task Should_update_user_profiel()
    {
        ...
        var command = Fixture.Build<UpdateUserProfile>()
        ...
        var sut = new UpdateUserProfileHandler(_repository);
        await sut.Handle(command);

        var dbEntity = await _context.UserProfiles.FirstOrDefaultAsync(x => x.Id == profile.Id);

        Assert.NotNull(dbEntity);
        Assert.AreEqual(command.SidebarLayout, dbEntity?.SidebarLayout);
    }
}