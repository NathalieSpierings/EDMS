...
public class AanleverbestandRepository : Repository<Aanleverbestand>, IAanleverbestandRepository
{
    ...
    /// </returns>
    public async Task<IList<Aanleverbestand>> GetAanleverbestandenByAanleveringId(Guid aanleveringId, ClaimsPrincipal user)
    {
        ...
        if (user.IsInRole(RoleNames.RaadplegenEigenAanleverbestanden))
        {
            return await Query()
                ...
                .Include(i => i.EiStandaard)
                .Include(i => i.Aanlevering)
                .Where(x => x.AanleveringId == aanleveringId && x.EigenaarId == Guid.Parse(user.FindFirst(ClaimTypes.NameIdentifier).Value))
                .ToListAsync();
        }
